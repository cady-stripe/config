...
def build_rename(i3, app_icons, args):
    ...
    def get_icon_or_name(leaf, length):
        if leaf.window_class == 'jetbrains-idea-ce' and leaf.window_title.startswith('kotlin '):
            branch = re.search('[^[]*\\[([^[]+)\\].*', leaf.window_title)
            if branch:
                return '\uf126 {}'.format(branch.group(1))
        ...
        for workspace in workspaces:
            ...
            if workspace_num >= 0:
                if workspace_num == 0:
                    ...
                    if workspace_num == 11:
                        workspace_key = 'q'
                    elif workspace_num == 12:
                        workspace_key = 'w'
                    elif workspace_num == 13:
                        workspace_key = 'e'
                    elif workspace_num == 14:
                        workspace_key = 'r'
                    elif workspace_num == 15:
                        workspace_key = 't'
                    elif workspace_num == 16:
                        workspace_key = 'y'
                    elif workspace_num == 17:
                        workspace_key = 'u'
                    elif workspace_num == 18:
                        workspace_key = 'i'
                    elif workspace_num == 19:
                        workspace_key = 'o'
                    elif workspace_num == 20:
                        workspace_key = 'p'
                    else:
                        workspace_key = workspace_num

                    if names == '':
                        newname = u"{}:{}".format(workspace.num, workspace_key)
                    else:
                        newname = u"{}:{}: {}".format(workspace.num, workspace_key, names)
            else:
                newname = names
...
def main():
    ...
    for case in ['window::move', 'window::new', 'window::title', 'window::close', 'workspace::focus']:
        i3.on(case, rename)
    i3.main()
